...
BOOST_AUTO_TEST_CASE( empty_script )
{
	...
	BOOST_CHECK_THROW(controle.onUserButton(1, 1), LuaCantCallFuncError);
}
...
BOOST_AUTO_TEST_CASE( simplest_script )
{
	...
	BOOST_CHECK_NO_THROW(controle.onUserButton(1, 1));
}
...
BOOST_AUTO_TEST_CASE( add_tracking_var )
{
	Ash ash;
	LuaControleScript controle(ash, "script/add_tracking_var.lua");
	BOOST_CHECK_NO_THROW(controle.initialize());
	controle.getSaveData(std::cout);
}
